...
>
> 当前流程已从旧的“三路并行计划确认 → 采集 → 完整性评估”调整为“真实意图识别 → 知识架构图谱生成 → Neo4j 首屏呈现 → 两轮架构 review / 自动修补 → 图谱补全 → 等待查询填充”。“查询填充”由用户点击按钮触发，之后才执行可信证据链接查询、治理质检和图谱证据写入；本地文档落地和版本研报继续属于后置可选动作。
...
```text
意图识别 -> 图谱生成 -> Neo4j呈现 -> 架构Review -> 图谱补全 -> 查询填充 -> 治理质检 -> 图谱证据写入 -> 可选补全文档 -> 可选版本研报
```
...
6. 第二轮仍不完整时，系统直接执行第二轮自动修补，将修补后的结构更新到 Neo4j，并继续进入图谱补全；不等待用户恢复或人工干预。
...
此步骤保存的是补全文档所需的结构化上下文。本地路径只作为建议路径或未来落盘路径保存，不代表文件已经存在。步骤完成后任务进入 `graph_ready`，前端显示完整 Neo4j 知识图谱，并等待用户点击“查询填充”。
