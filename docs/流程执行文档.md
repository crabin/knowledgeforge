...
>
> 当前流程已从旧的“三路并行计划确认 → 采集 → 完整性评估”调整为“真实意图识别 → 知识架构图谱生成 → Neo4j 首屏呈现 → 两轮架构 review / 自动修补 → 图谱补全 → 可信证据链接查询 → 治理质检 → 等待补全文档”。“图谱证据写入”是补全文档前的可选动作，不在默认主链路执行。
...
flowchart TD
    ...
    P --> Q{用户是否点击补全文档}
    Q -->|否| R[结束 / 可选研报]
    Q -->|是| K[record_evidence_to_graph 图谱证据写入]
    K --> T[检查图谱 review / 证据 / 治理状态]
...
7. 更新运行态队列和 SSE payload；不写入 Neo4j 证据字段。
...
触发点：用户点击 `/tasks/{task_id}/documents/complete` 后、生成本地 Markdown 前。
...
- 补全文档触发证据写入后找到可访问链接：`link_verified`
- 补全文档触发证据写入后无合格链接或异常：`link_failed`
- 架构阶段不执行父级状态聚合。
