...
>
> 当前流程已从旧的“三路并行计划确认 → 采集 → 完整性评估”调整为“真实意图识别 → 知识架构图谱生成 → Neo4j 首屏呈现 → 两轮架构 review / 自动修补 → 图谱补全 → 可信证据链接查询 → 治理质检 → 图谱证据写入 → 等待补全文档”。“图谱证据写入”是默认主链路的 Neo4j 必需能力；只有本地文档落地和版本研报属于后置可选动作。
