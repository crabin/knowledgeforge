...
>
> 当前流程已从旧的“三路并行计划确认 → 采集 → 完整性评估”调整为“真实意图识别 → 结构图谱规划 → 串行文件生成 → 文件级证据队列 → 即时回写 → 治理质检”。
...
flowchart TD
    ...
    D --> E[同步结构图谱到 Neo4j]
    E --> F[generate_files 串行生成 Markdown]
...
| `structure_graph` | LLM 或 fallback 生成的知识结构图谱 |
...
4. 若 LLM 失败，生成 fallback Markdown。
5. 写入目标 `.md` 文件。
...
- 作为收尾校验和兼容层。
- 不再承担“主流程证据统一回填”的职责。
- 主要用于生成 `QueueFillPass` 输出和兼容后置治理链路。
...
- 当前主线是文件级证据队列，Query / Media 按任务执行。
