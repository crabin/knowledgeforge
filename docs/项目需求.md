...
- **可选完整知识文档**：基于已完成的架构图谱、架构文档和可信链接补全完整 Markdown 知识库文档。
- **后置补全文档动作**：Web UI 不在 intake 阶段选择完整文档；用户在架构与链接流程完成后点击“补全文档”，系统先检查架构 review、链接队列和治理状态，再逐个补全 Markdown。
...
- `completion_mode` 默认为 `framework`；Web UI 始终先生成框架证据，完整知识文档通过后置“补全文档”动作触发；旧值 `file_level` 兼容为 `full_document`。
