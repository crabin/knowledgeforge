...
> **当前真实目标**：输入一个领域或自然语言描述后，系统先识别真实意图和规范领域名，再由 LLM 生成知识架构图谱、优先同步到 Neo4j 首屏呈现、执行“结构覆盖审查 + 执行准备度审查 + 结构深化审查”的三段 review 与自动修补。默认主链路先停在完整 Neo4j 知识图谱；用户点击“查询填充”后才联网补充证据、治理质检并把已验证证据写入 Neo4j；“补全文档”和“生成研报”是后置可选动作。

## 1. 项目定位

KnowledgeForge 是一个面向领域知识沉淀的知识工程系统。

系统以用户输入的领域或描述为入口，通过 intake 识别真实意图，使用 LangGraph 编排“知识架构图谱生成 → Neo4j 呈现 → 结构覆盖审查 / 自动修补 → 图谱补全文档上下文准备 → 执行准备度审查 / 自动修补 → 结构深化审查 / 自动修补 → 图谱完成”的图谱生成闭环，先形成可追溯、可迭代、可图谱化展示的领域知识架构。可信证据链接查询、治理质检和 Neo4j 图谱证据写入由后置“查询填充”动作触发；完整本地知识库文档只由后置“补全文档”动作触发。
...
5. 支持三段 LLM review：第一段 `structure_coverage` 只审知识图谱结构覆盖；第二段 `completion_readiness` 在补全文档上下文准备后审证据需求、query 任务、建议路径和治理准备度；第三段 `structure_depth` 聚焦倒数第二级节点是否过薄、叶子节点是否过宽，必要时补充分支或拆分叶子。每段发现缺口后自动修补并同步 Neo4j，后两段仍不完整时也直接使用自动修补后的图谱继续主链路，不等待人工恢复。
...
flowchart TD
    ...
    N --> H2[重新准备图谱补全文档上下文]
    H2 --> K[结构深化审查 structure_depth]
    G -->|通过| K
    K -->|有必要深化| Z[深化修补并同步 Neo4j]
    Z --> H3[重新准备图谱补全文档上下文]
    H3 --> I
    K -->|通过| I[生成图谱级证据任务 / 等待查询填充]
    I --> U{用户是否点击查询填充}
...
generate_structure_graph
  ...
  -> review_structure_round_3(structure_depth)
  -> repair_structure_graph_round_3(可选)
  -> prepare_graph_completion_context(仅第三段修补后重跑)
...
架构阶段不做父级完成聚合；父级是否完整、可执行准备度与分支深化质量由三段 review 的结果决定。补全文档阶段如需展示落盘进度，可在补全文档任务中另行聚合。
...
- 默认主链路检查知识架构图谱完整度、三段 review 结果、官方/权威链接覆盖、图谱路径关联和补全文档上下文状态。
...
4. 三段 review 均有 `structure_review_rounds` 记录，并通过 `review_type` 区分 `structure_coverage`、`completion_readiness` 与 `structure_depth`；后两段不通过时系统自动修补、同步 Neo4j，并继续图谱完成，不要求用户手动恢复。
...
7. 架构阶段不做父级完成聚合；架构完整性、执行准备度与分支深化质量由三段 review 决定。
