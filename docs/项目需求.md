...
> **当前真实目标**：输入一个领域或自然语言描述后，系统先识别真实意图和规范领域名，再由 LLM 生成知识架构图谱、优先同步到 Neo4j 首屏呈现、执行两轮自动 review 与修补。默认主链路只完善 Neo4j 知识图谱、运行态证据链接和治理状态，不生成本地知识 Markdown；只有用户点击“补全文档”后，才先把已验证证据写入 Neo4j，再基于图谱批量生成本地文档。

## 1. 项目定位

KnowledgeForge 是一个面向领域知识沉淀的知识工程系统。

系统以用户输入的领域或描述为入口，通过 intake 识别真实意图，使用 LangGraph 编排“知识架构图谱生成 → Neo4j 呈现 → 两轮架构 review / 自动修补 → 图谱补全 → 可信证据链接查询 → 治理质检 → 等待补全文档”的闭环，先形成可追溯、可迭代、可图谱化展示的领域知识架构。图谱证据写入和完整本地知识库文档只由后置“补全文档”动作触发。
...
5. 支持两轮 LLM 架构 review；每轮发现缺口后自动修补图谱，第二轮仍不完整则进入 `repair_required`，不生成本地知识 Markdown。
6. 默认支持审查通过后把知识点蓝图、证据需求、建议路径和补全文档上下文写入 Neo4j，而不是立即落本地 Markdown。
7. 支持 QueryEngine 可信链接查询；主链路不执行 MediaEngine 证据任务。
8. 支持每条链接任务完成后更新运行态任务状态和 SSE payload；补全文档触发前不把证据字段写入 Neo4j，也不把网页内容、摘要或架构正文即时写回 Markdown。
...
- **图谱证据上下文**：默认先记录在运行态证据队列；用户点击“补全文档”后，再写入 Neo4j 节点字段 `evidence_links`、`selected_link`、`source_kind`、`reachable`、`relevance_reason`、`checked_at`、`claim_or_gap`、`expected_evidence`、`review_status`、`repair_log`、`suggested_relative_path`、`document_completion_status`。
...
flowchart TD
    ...
    V --> P{用户是否点击补全文档}
    P -->|否| R[结束 / 可选研报]
    P -->|是| K[Neo4j 写入 selected_link / reachable / source_kind]
    K --> Q[检查图谱证据和治理状态]
...
架构阶段不做父级完成聚合；父级是否完整只由两轮架构 review 的结果决定。补全文档阶段如需展示落盘进度，可在补全文档任务中另行聚合。
...
- 默认主链路检查知识架构图谱完整度、两轮 review 结果、官方/权威链接覆盖、图谱路径关联和补全文档上下文状态。
...
```text
意图识别 -> 图谱生成 -> Neo4j呈现 -> 架构Review -> 图谱补全 -> 证据链接 -> 治理质检 -> 可选图谱证据写入 -> 补全文档
```
...
4. 两轮架构 review 均有 `structure_review_rounds` 记录；第二轮不通过时任务进入 `repair_required` 且不生成本地知识 Markdown。
