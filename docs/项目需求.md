...
> **当前真实目标**：输入一个领域或自然语言描述后，系统先识别真实意图和规范领域名，再由 LLM 生成知识架构图谱、优先同步到 Neo4j 首屏呈现、执行两轮自动 review 与修补。默认主链路先停在完整 Neo4j 知识图谱；用户点击“查询填充”后才联网补充证据、治理质检并把已验证证据写入 Neo4j；“补全文档”和“生成研报”是后置可选动作。

## 1. 项目定位

KnowledgeForge 是一个面向领域知识沉淀的知识工程系统。

系统以用户输入的领域或描述为入口，通过 intake 识别真实意图，使用 LangGraph 编排“知识架构图谱生成 → Neo4j 呈现 → 两轮架构 review / 自动修补 → 图谱补全”的图谱生成闭环，先形成可追溯、可迭代、可图谱化展示的领域知识架构。可信证据链接查询、治理质检和 Neo4j 图谱证据写入由后置“查询填充”动作触发；完整本地知识库文档只由后置“补全文档”动作触发。
