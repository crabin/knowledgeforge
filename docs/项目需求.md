...
> **当前真实目标**：输入一个领域或自然语言描述后，系统先识别真实意图和规范领域名，再由 LLM 生成知识架构图谱、优先同步到 Neo4j 首屏呈现、执行两轮自动 review 与修补。默认主链路只完善 Neo4j 知识图谱、证据链接和治理状态，不生成本地知识 Markdown；只有用户点击“补全文档”后，才基于图谱和已验证链接批量生成本地文档。

## 1. 项目定位

KnowledgeForge 是一个面向领域知识沉淀的知识工程系统。

系统以用户输入的领域或描述为入口，通过 intake 识别真实意图，使用 LangGraph 编排“知识架构图谱生成 → Neo4j 呈现 → 两轮架构 review / 自动修补 → 图谱补全 → 可信证据链接查询 → 图谱证据写入 → 治理质检 → 等待补全文档”的闭环，先形成可追溯、可迭代、可图谱化展示的领域知识架构。完整本地知识库文档只由后置“补全文档”动作生成。
...
8. 支持每条链接任务完成后更新任务状态、Neo4j 目标节点和 SSE payload；不把网页内容、摘要或架构正文即时写回 Markdown。
...
- **图谱证据上下文**：在 Neo4j 节点上记录 `evidence_links`、`selected_link`、`source_kind`、`reachable`、`relevance_reason`、`checked_at`、`claim_or_gap`、`expected_evidence`、`review_status`、`repair_log`、`suggested_relative_path`、`document_completion_status`。
...
flowchart TD
    ...
    J --> K[Neo4j 写入 selected_link / reachable / source_kind]
...
6. 每个链接任务完成后，Neo4j 节点记录 `selected_link`、`reachable`、`source_kind`、`checked_at` 和关联 claim。
