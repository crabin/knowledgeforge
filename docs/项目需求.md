...
5. 支持两轮 LLM 架构 review；每轮发现缺口后自动修补图谱并同步 Neo4j，第二轮仍不完整时也直接使用自动修补后的图谱继续主链路，不等待人工恢复。
...
```text
意图识别 -> 图谱生成 -> Neo4j呈现 -> 架构Review -> 图谱补全 -> 证据链接 -> 治理质检 -> 图谱证据写入 -> 可选补全文档 -> 可选版本研报
```
...
4. 两轮架构 review 均有 `structure_review_rounds` 记录；第二轮不通过时系统自动修补结构、同步 Neo4j，并继续图谱补全，不要求用户手动恢复。
