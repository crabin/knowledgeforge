...
> **当前真实目标**：输入一个领域或自然语言描述后，系统先识别真实意图和规范领域名，再生成领域知识框架图谱、初始化 Neo4j 任务图、串行生成框架证据 Markdown 文件、按文件级证据队列补充官方或权威来源，并在每个状态变化时通过 SSE 同步前端。完整知识库文档和研报生成是框架证据完成治理后的可选能力。

## 1. 项目定位

KnowledgeForge 是一个面向领域知识沉淀的知识工程系统。

系统以用户输入的领域或描述为入口，通过 intake 识别真实意图，使用 LangGraph 编排“知识框架图谱规划 → 框架证据文件生成 → 证据查询 → 即时回写 → 治理质检 → 版本冻结”的闭环，先形成可追溯、可迭代、可图谱化展示的领域知识框架。完整知识库文档可在最后基于框架、证据和学习进度补全。
...
5. 默认支持串行生成每个知识点的框架证据 Markdown 文件，并保存证据占位和 `query_tasks` contract。
...
- **本地框架证据库**：按领域 / 子领域 / 知识点组织，默认保存为框架证据 Markdown。
...
- **可选完整知识文档**：在 `completion_mode=full_document` 时，基于已完成的框架图谱和证据文件补全完整 Markdown 知识库文档。
...
flowchart TD
    ...
    C --> D[Neo4j 前置同步知识框架图谱 planned]
    D --> E[串行生成框架证据 Markdown 文件]
...
- 每个框架证据文件必须能回溯到来源、Agent、轮次、时间、本地路径和证据任务。
- `completion_mode` 默认为 `framework`；`full_document` 表示在框架证据治理后补全完整知识文档；旧值 `file_level` 兼容为 `full_document`。
...
默认 `framework` 模式下，每个知识点文件是框架证据文件，必须包含：
...
- 官方文档、标准、规范、权威论文或项目主页优先的证据任务。

可选 `full_document` 模式下，系统在框架证据治理后再生成完整知识库文档，完整文档必须包含：
...
- `framework` 模式检查知识框架图谱完整度、框架证据文件存在性、官方/权威来源覆盖、路径关联和 contract 状态。
