...
## 2026-05-05 图谱生成与查询填充分步发现
- 用户反馈“证据为 0”不是异常，而是流程语义需要改清楚：第一步应先生成完整 Neo4j 知识图谱，证据补充是下一步用户主动触发的联网动作。
...
- 2026-05-06 新决策：批量待查内容可以先由 LLM 按权威来源优先级表分类，生成 `preferred_source_types/source_priority/authority_queries/acceptance_criteria`，再落为运行态 JSON 查询队列；这个步骤属于查询任务准备辅助，不改变主链路的 Neo4j-first 与后置补全文档边界。
...
## 图谱证据写入必需化结论
- 08「图谱证据写入」不是可选文档动作，而是 Neo4j 主链路的必需能力；只有 09「补全文档」和 10「版本研报」保持可选。
- 证据链接查询可以继续先写运行态队列，但验证和治理后必须自动把 `selected_link`、`source_kind`、`reachable`、`relevance_reason`、`checked_at`、`claim_or_gap` 和 `expected_evidence` 同步到 Neo4j 结构节点。
- 本地 Markdown 仍不应在默认主链路落地；补全文档按钮只消费已写入 Neo4j 的证据和图谱上下文生成 README / 知识点文档。
